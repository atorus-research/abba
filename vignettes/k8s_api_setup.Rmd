...
If users encounter issues that they can't correct themselves based off of errors within the end user function, you can check the API logs to look for more information.  On the top toolbar of the **abba** API deployed in Posit Connect, click the notebook page icon to open the logs.
...
In this example, you can see that the `kubectl` function is getting permission denied. This is because the user account was not yet switched to use an account that has `kubectl` configured.
